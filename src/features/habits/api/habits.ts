...
export async function setHabitComplete(habitId:string,date:string,complete:boolean){const result=complete?await dataApi.from('habit_completions').upsert({habit_id:habitId,completion_date:date},{onConflict:'user_id,habit_id,completion_date'}):await dataApi.from('habit_completions').delete().eq('habit_id',habitId).eq('completion_date',date);if(result.error)throw fail(result.error);notify()}
