...
export function DashboardTasksCard({ mobile = false }: { mobile?: boolean }) {
  ...
  if (mobile) return <section className="card tasks-card mobile-today-card">
    <div className="card-head"><div><h2>Today’s priorities</h2><span className="dashboard-card-caption">Due and overdue tasks</span></div><Link className="meta" to="/tasks">View all</Link></div>
    {error ? <p className="meta">Project data unavailable.</p> : actionable.length ? <div className="task-list">{actionable.map(task => <label className="task" key={task.id}><input type="checkbox" checked={false} onChange={() => void setTaskCompleted(task.id, true)} /><span><small className="task-path">{taskCategoryLabels[task.category]} · {ddayLabel(task.dueDate)}</small>{task.title}</span></label>)}</div> : <p className="meta">No tasks due today.</p>}
    {pinned.length > 0 && <div className="mobile-pinned-deadlines"><span className="dashboard-card-caption">Next pinned deadlines</span>{pinned.slice(0, 2).map(item => <Link to={projectPath(item.kind, item.id)} key={`${item.kind}-${item.id}`}><span>{item.title}</span><b className="tabular">{ddayLabel(item.dueDate)}</b></Link>)}</div>}
  </section>
