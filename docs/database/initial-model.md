...
| --- | --- | --- |
| `study_sessions` | Implemented: completed timed/manual intervals and one sentinel active timer row | No subject foreign key by design; owner-only RLS; a partial unique index permits one active timer per owner |
| `study_timer_state` | Compatibility table introduced by migration 0024 | Superseded by the active `study_sessions` row so clients use the already-established Data API resource |
...
| `study_timetables` / `study_timetable_courses` / `study_class_meetings` | Implemented: imported semester timetable and normalized weekly class blocks | Courses reuse StudyOS Subjects; one imported timetable per owner/year/term |
