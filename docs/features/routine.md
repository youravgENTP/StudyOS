...
Template creation and editing happen in expanding rows within the timeline. Template rows support handle-based drag reorder persisted on drop, with up/down controls retained as a keyboard-friendly fallback. Today’s snapshot is not reordered by template changes. The Dashboard only projects completion counts from the current instance.
