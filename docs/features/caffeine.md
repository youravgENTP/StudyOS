...
The header places the current estimate, projected total at bedtime, and dose-aware guidance for the built-in double-shot reference in one horizontal row aligned with—not above—the page title. On narrow screens, that complete horizontally scrollable row moves below the title rather than stacking its metrics. `calculateLatestAllowableIntakeTime` adds that hypothetical intake to every loaded recorded intake and searches for the latest start that finishes before bedtime while keeping `totalLoadAt` at or below the planning target. Existing caffeine therefore consumes the available bedtime budget; if it already reaches the target, the UI reports that no additional caffeine fits. Changes to intake records, dose/duration, bedtime, half-life, or the target recompute the result. This replaces the former fixed bedtime-minus-eight-hours recommendation.

The Intake Log includes previous/next-day controls and a calendar date picker; its total and rows follow the selected local date. On desktop, a draggable and keyboard-operable separator changes the width allocated to Presets and the Intake Log and persists that device-local split. Narrow layouts stack the panels instead.
